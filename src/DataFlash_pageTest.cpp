...
DataFlash dataflash_pt;
uint8_t   loop_cnt;
uint16_t  page;

void setup_dataflash_at45d_pageTest()
{
  uint8_t status;
  DataFlash::ID id;

  Serial.println("Dataflash AT45D example");
  ...
  /* Initialize dataflash_pt */
  dataflash_pt.setup(10,11,12);
  ...
  status = dataflash_pt.status();

  /* Read manufacturer and device ID */
  dataflash_pt.readID(id);
...
void loop_dataflash_at45d_pageTest()
{
  unsigned int i,j;
  ...
  dataflash_pt.bufferToPage(1, page);
  ...
  if(page >= NUM_PAGES)
  {
    for(i=0; i<NUM_PAGES; ++i)
    {
      if(i & 1)
      {
        ...
        dataflash_pt.pageToBuffer(i, 1);
        dataflash_pt.bufferRead(1, 0);
      }
      else {
        Serial.println("Page read");
        dataflash_pt.pageRead(i, 0);
      }
